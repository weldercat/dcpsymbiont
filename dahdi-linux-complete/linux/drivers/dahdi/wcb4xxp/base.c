...
#define DEBUG_ME_HARDER		0		/* change to 1 for more verbosity */
...
/* uncomment to see CICHAN data via bchans */
//#define DEBUG_SNOOP_CICHAN		1

static int debug = 0;
static int spanfilter = 0xFFFF; /* Bitmap for ports 1-16 */
#ifdef LOOPBACK_SUPPORTED
static int loopback = 0;
#endif
static unsigned int dbgval1 = 0;
static unsigned int dbgval2 = 0;
static int milliwatt = 0;
static int hw_dtmf = 0;
static int hw_rxmute = 0;
static int dtmfthreshold = 100;
static int dchan_dtmfevt = 0;
...
static int master = 0;
static int pcmbus_devs = 0;
static int pcm_loop = 0;
static int pcm_slot_loop = 0;
static int pcm_synci = 0;
static int pcm_c4_pol = 0;
static int pcm_f0_neg = 0;
static int pcm_f0_len = 0;
...
/* these two tables are OK for HFC4-S only */
static const int pcmbus_fifo_table[] = {
	-1, -1, -1, -1, 
	12, 16, 19, 22, 25, -1, -1,
	-1, -1, -1, -1, -1
};

static const int pcmbus_chan_table[] = {
	-1, -1, -1, -1,
	16, 19, 22, 25, 28, -1, -1
	-1, -1, -1, -1, -1
};
...
#if 0
static const unsigned char mwt_ulaw[] = {
	0x1e, 0x0b, 0x0b, 0x1e,
	0x9e, 0x8b, 0x8b, 0x9e
};

static const unsigned char mwt_alaw[] = {
	0x34, 0x21, 0x21, 0x34, 
	0xb4, 0xa1, 0xa1, 0xb4
};
#endif 
...
static const unsigned int mwt_ulaw[] = {
	0x1e0b0b1e,
	0x9e8b8b9e
};

static const unsigned int mwt_alaw[] = {
	0x34212134, 
	0xb4a1a1b4
};
#else 
#error Milliwatt tables are hardcoded for little-endianness - please review and correct
#endif

#define NCOEFF	8

static uint64_t cos2pik[NCOEFF] =
{
...
{
	{DAHDI_TONE_DTMF_1, DAHDI_TONE_DTMF_2, DAHDI_TONE_DTMF_3, DAHDI_TONE_DTMF_A},
	{DAHDI_TONE_DTMF_4, DAHDI_TONE_DTMF_5, DAHDI_TONE_DTMF_6, DAHDI_TONE_DTMF_B},
	{DAHDI_TONE_DTMF_7, DAHDI_TONE_DTMF_8, DAHDI_TONE_DTMF_9, DAHDI_TONE_DTMF_C},
	{DAHDI_TONE_DTMF_s, DAHDI_TONE_DTMF_0, DAHDI_TONE_DTMF_p, DAHDI_TONE_DTMF_D}
};

static char tone_ascii_tab[16] = "0123456789*#ABCD";


#define CREATE_WCB4XXP_PROCFS_ENTRY	1
#ifdef CREATE_WCB4XXP_PROCFS_ENTRY
#define PROCFS_NAME 		"driver/wcb4xxp"
static struct proc_dir_entry *myproc;
#endif

/* Expansion; right now there's just one card and all of its idiosyncrasies. */

#define FLAG_CANPCMBUS	(1 << 0)
#define FLAG_CANHWDTMF	(1 << 1)
#define FLAG_zzz	(1 << 2)
...
static struct devtype hfc4s =	 {"HFC-4S Junghanns.NET quadBRI PCI", .flags = (FLAG_CANPCMBUS | FLAG_CANHWDTMF) ,.ports = 4, .card_type = QUADBRI };
...
#define CAN_PCMBUS_DEVS(card) ((card)->card_type == QUADBRI)
#define CARD_HAS_EC(card) ((IS_B410P(card) || IS_B430P(card) || IS_B230P(card)) && (!CAN_PCMBUS_DEVS(card)))

static void echocan_free(struct dahdi_chan *chan, struct dahdi_echocan_state *ec);
static void b4xxp_update_leds(struct b4xxp *b4);
static void hfc_handle_ibc_state(struct b4xxp_span *span);
#ifdef CREATE_WCB4XXP_PROCFS_ENTRY
static const char *ibc_modename(enum ibc_mode md);
static const char *ibc_statename(enum ibc_state st);
#endif
static void set_dtmf_addr(struct b4xxp *b4, unsigned int co, unsigned int ch);
...
/*
 * returns true if this span is synthetic (accessed via PCMBUS)
 *
 */

static inline bool is_synthetic(struct b4xxp_span *bspan)
{
	bool synthetic;
	synthetic = ((bspan->flags && SPAN_FLAG_PCMBUS) || bspan->phy_port < 0);
	return synthetic;
}
...
static int unpack_dtmf_float(uint16_t w)
{
	unsigned int mantissa;
	uint8_t	exponent;

	mantissa = w & 0x0fff;
	if (w & 0x8000) mantissa |= 0xfffff000;
	exponent = (w >> 12) & 0x7;
	if (exponent) {
		mantissa ^= 0x1000;
		mantissa <<= (exponent - 1);
...
static void set_dtmf_addr(struct b4xxp *b4, unsigned int co, unsigned int ch)
{
	...
	addr = b4->dtmf.base + ((co << 7) | (ch << 2));
	b4xxp_setreg8(b4, R_RAM_ADDR0, addr & 0xff);
	b4xxp_setreg8(b4, R_RAM_ADDR1, (addr >> 8) & 0xff);
	b4xxp_setreg8(b4, R_RAM_ADDR2, ((addr >> 16) & 0xff) | V_ADDR_INC);
}

static inline uint16_t read_dtmf_float(struct b4xxp *b4)
{
	uint16_t w_float;

	w_float = b4xxp_getreg8(b4, R_RAM_DATA);
	w_float |= (b4xxp_getreg8(b4, R_RAM_DATA) << 8);
	return w_float;
}
...
static inline unsigned int pcm_clock_mode(void)
{
	unsigned int mode;

	mode = 0;
	if (pcm_c4_pol) mode |= V_C4_POL;
	if (pcm_f0_neg) mode |= V_F0_NEG;
	if (pcm_f0_len) mode |= V_F0_LEN;
	return mode;
}
 ...
 */
	...
	b4xxp_setreg8(b4, R_PCM_MD0, (master ? V_PCM_MD : 0) | V_PCM_IDX_MD1 | pcm_clock_mode());
	flush_pci();

	if (IS_GEN2(b4))
		b4xxp_setreg8(b4, R_PCM_MD1, V_PLL_ADJ_00 | V_PCM_DR_4096 | (pcm_loop ? V_PCM_LOOP : 0));
	else
		b4xxp_setreg8(b4, R_PCM_MD1, V_PLL_ADJ_00 | V_PCM_DR_2048 | (pcm_loop ? V_PCM_LOOP : 0));
	b4xxp_setreg8(b4, R_PCM_MD2, (pcm_synci ? V_SYNC_SRC : 0));
	flush_pci();
...
/* clear any pending interrupts */
	...
	if (hw_dtmf) {
		misc_irqmsk = (V_DTMF_IRQ | V_TI_IRQ);
		b4xxp_setreg8(b4, R_DTMF_N, DSP_DTMF_NPOINTS - 1);
		b4xxp_setreg8(b4, R_DTMF, b4->dtmf.cfg | V_RST_DTMF);
	} else {
		misc_irqmsk = V_TI_IRQ;
	};

	b4xxp_setreg8(b4, R_IRQMSK_MISC, misc_irqmsk);
...
static void hfc_init_chan_dtmf(struct b4xxp *b4, int port, int bchan, int hfc_chan)
{
	struct chan_dtmf cd;
	...
	cd.chan = hfc_chan;
	cd.dtmfenable = false;
	if (dtmfthreshold < 20 || dtmfthreshold > 500) dtmfthreshold = 200;
	cd.threshold = dtmfthreshold * 10000;
	b4->spans[port].tonecfg[bchan] = cd;
	if (unlikely(DBG_DTMF)) {
		printk("dtmf init for port: %d, bchan: %d, hfc_chan: %d\n",
			port, bchan, hfc_chan);
	}
}
...
/* record the host's FIFO # in the span fifo array */
	...
	hfc_init_chan_dtmf(b4, port, bchan, hfc_chan);
...
static void hfc_assign_fifo_zl(struct b4xxp *b4, int port, int bchan)
{
	...
	if (IS_B230P(b4)) {
		...
		hfc_init_chan_dtmf(b4, 2-port, bchan, hfc_chan);

	} else {
		b4->spans[3-port].fifos[bchan] = fifo;
		hfc_init_chan_dtmf(b4, 3-port, bchan, hfc_chan);
	}
...
static void hfc_assign_bchan_fifo_pcmbus(struct b4xxp *b4, int port, int bchan)
{
	int fifo, hfc_chan, ts;
	unsigned long irq_flags;


	fifo = pcmbus_fifo_table[port];
	hfc_chan = pcmbus_chan_table[port];
	ts = (port - 4) * 4;

	if ((fifo < 0) || (hfc_chan < 0)) {
		dev_err(&b4->pdev->dev, "Unable to configure PCMBUS span %d, B%d - no free FIFOs or chans\n",
			port, bchan);
		return;
	}
	if (bchan) {
...
/* record the host's FIFO # in the span fifo array */
	...
	b4->spans[port].flags |= SPAN_FLAG_PCMBUS;
	spin_lock_irqsave(&b4->fifolock, irq_flags);
	...
	b4xxp_setreg8(b4, A_CHANNEL, (hfc_chan << V_CH_FNUM_SHIFT));
	b4xxp_setreg8(b4, R_SLOT, (ts << V_SL_NUM_SHIFT));
	b4xxp_setreg8(b4, A_SL_CFG, (pcm_slot_loop ? V_ROUT_TX_LOOP : V_ROUT_TX_STIO1) | (hfc_chan << V_CH_SNUM_SHIFT));
	hfc_setreg_waitbusy(b4, A_INC_RES_FIFO, V_RES_FIFO);
	...
	b4xxp_setreg8(b4, A_CHANNEL, (hfc_chan << V_CH_FNUM_SHIFT) | V_CH_FDIR);
	b4xxp_setreg8(b4, R_SLOT, (ts << V_SL_NUM_SHIFT) | V_SL_DIR);
	b4xxp_setreg8(b4, A_SL_CFG, (pcm_slot_loop ? V_ROUT_RX_LOOP : V_ROUT_RX_STIO2) | (hfc_chan << V_CH_SNUM_SHIFT) | V_CH_SDIR);
	hfc_setreg_waitbusy(b4, A_INC_RES_FIFO, V_RES_FIFO);
...
static void hfc_assign_cichan_fifo_pcmbus(struct b4xxp *b4, int port)
{
	int fifo, hfc_chan, ts;
	unsigned long irq_flags;
	...
	if ((fifo < 0) || (hfc_chan < 0)) {
		dev_err(&b4->pdev->dev, "Unable to configure PCMBUS span %d, C/Ichan - no free FIFOs or chans\n",
			port);
		return;
	}

	fifo += 3;
	hfc_chan += 2;	//same as d-channel
	b4->spans[port].fifos[WCB4XXP_CICHAN_FIFO_IDX] = fifo;
	b4->spans[port].flags |= (SPAN_FLAG_PCMBUS | SPAN_FLAG_CI);

	if (DBG) {
		dev_info(&b4->pdev->dev,
			 "port %d, C/I channel\n\thost -> PCM ts %d uses HFC chan %d "
			 "via FIFO %d\n", port, ts, hfc_chan, fifo);
	}

	spin_lock_irqsave(&b4->fifolock, irq_flags);

/* Host FIFO -> PCM TX, TRANSPARENT mode, no IRQ. */
	hfc_setreg_waitbusy(b4, R_FIFO, (fifo << V_FIFO_NUM_SHIFT));
	b4xxp_setreg8(b4, A_CON_HDLC, V_IFF | V_HDLC_TRP | V_DATA_FLOW_001);

	b4xxp_setreg8(b4, A_CHANNEL, (hfc_chan << V_CH_FNUM_SHIFT));
/* slot must be already configured by assign_dchan_fifo_pcmbus() */
//#if 0	
	b4xxp_setreg8(b4, R_SLOT, (ts << V_SL_NUM_SHIFT));
	b4xxp_setreg8(b4, A_SL_CFG, V_ROUT_TX_STIO1 | (hfc_chan << V_CH_SNUM_SHIFT));
//#endif 
	b4xxp_setreg8(b4, A_SUBCH_CFG, 0x04 | (2 << V_START_BIT_SHIFT));
	b4xxp_setreg8(b4, A_CH_MSK, (0xff));
	hfc_setreg_waitbusy(b4, A_INC_RES_FIFO, V_RES_FIFO);

	if (DBG)
		pr_info("\tPCM ts %d -> host uses HFC chan %d via FIFO %d\n", ts, hfc_chan, fifo);

/* PCM RX -> Host FIFO, TRANSPARENT mode, no IRQ */
	hfc_setreg_waitbusy(b4, R_FIFO, (fifo << V_FIFO_NUM_SHIFT) | V_FIFO_DIR);
	b4xxp_setreg8(b4, A_CON_HDLC, V_IFF | V_HDLC_TRP | V_DATA_FLOW_001);
	b4xxp_setreg8(b4, A_CHANNEL, (hfc_chan << V_CH_FNUM_SHIFT) | V_CH_FDIR);
/* slot must be already configured by assign_dchan_fifo_pcmbus() */
//#if 0
	b4xxp_setreg8(b4, R_SLOT, (ts << V_SL_NUM_SHIFT) | V_SL_DIR);
	b4xxp_setreg8(b4, A_SL_CFG, V_ROUT_RX_STIO2 | (hfc_chan << V_CH_SNUM_SHIFT) | V_CH_SDIR);
//#endif
	b4xxp_setreg8(b4, A_SUBCH_CFG, 0x00 | (0 << V_START_BIT_SHIFT));
	hfc_setreg_waitbusy(b4, A_INC_RES_FIFO, V_RES_FIFO);
...
static void hfc_assign_dchan_fifo_pcmbus(struct b4xxp *b4, int port)
{
	int fifo, hfc_chan, ts;
	unsigned long irq_flags;
	...
	ts = ((port - 4) * 4) + 3;
	if ((fifo < 0) || (hfc_chan < 0)) {
		dev_err(&b4->pdev->dev, "Unable to configure PCMBUS span %d, Dchan - no free FIFOs or chans\n",
			port);
		return;
	}
	...
	fifo += 2;
	hfc_chan += 2;
//	ts++; hfc_chan++;	//debug - must be commented out
	b4->spans[port].fifos[WCB4XXP_DCHAN_FIFO_IDX] = fifo;
	b4->spans[port].flags |= SPAN_FLAG_PCMBUS;
...
/* Host FIFO -> PCM TX, HDLC mode, no IRQ. */
	hfc_setreg_waitbusy(b4, R_FIFO, (fifo << V_FIFO_NUM_SHIFT));
	b4xxp_setreg8(b4, A_CON_HDLC, V_IFF | V_TRP_IRQ | V_DATA_FLOW_001);

	b4xxp_setreg8(b4, A_CHANNEL, (hfc_chan << V_CH_FNUM_SHIFT));
	b4xxp_setreg8(b4, R_SLOT, (ts << V_SL_NUM_SHIFT));
	b4xxp_setreg8(b4, A_SL_CFG, V_ROUT_TX_STIO1 | (hfc_chan << V_CH_SNUM_SHIFT));
	b4xxp_setreg8(b4, A_SUBCH_CFG, V_BIT_CNT_2BIT | (0 << V_START_BIT_SHIFT));
	b4xxp_setreg8(b4, A_CH_MSK, (0xff));
	hfc_setreg_waitbusy(b4, A_INC_RES_FIFO, V_RES_FIFO);
...
/* PCM RX -> Host FIFO, HDLC mode, IRQ will be enabled when port opened. */
	hfc_setreg_waitbusy(b4, R_FIFO, (fifo << V_FIFO_NUM_SHIFT) | V_FIFO_DIR);
	b4xxp_setreg8(b4, A_CON_HDLC, V_IFF | V_TRP_IRQ | V_DATA_FLOW_001);
	b4xxp_setreg8(b4, A_CHANNEL, (hfc_chan << V_CH_FNUM_SHIFT) | V_CH_FDIR);
	b4xxp_setreg8(b4, R_SLOT, (ts << V_SL_NUM_SHIFT) | V_SL_DIR);
...
static void hfc_init_all_st(struct b4xxp *b4)
{
	...
	for (i=0; i < b4->numspans; i++) {
		...
		if (s->flags & SPAN_FLAG_PCMBUS) {
			s->phy_port = -1;
			s->port = i;

#warning Hardcoded TE/NT selection for PCMBUS span
			nt = 1;
			s->te_mode = !nt;
			dev_info(&b4->pdev->dev,
				"Port %d (PCMBUS): %s mode\n", i + 1, (nt ? "NT" : "TE"));
			continue;
		}
...
static inline void ibc_cmd(struct b4xxp_span *span, unsigned char cmd)
{
	span->ci_cmd = cmd;
	span->cmd_sent = false;
	span->ind_count = 0;
...
static void b4xxp_update_ibc(struct b4xxp *b4)
{
	int	i;
	struct b4xxp_span *span;
	
	for (i=0; i < b4->numspans; i++) {
		span = &b4->spans[i];
		if (is_synthetic(span)) hfc_handle_ibc_state(span);
	}
}


static void hfc_new_ibc_mode(struct b4xxp_span *span, enum ibc_mode mode)
{
	struct b4xxp *b4;
	unsigned long irq_flags;
...
static void hfc_handle_ibc_state(struct b4xxp_span *span)
{
	...
	b4 = span->parent;
	spin_lock_irqsave(&b4->fifolock, irq_flags);
	switch (span->ibcstate) {
		case RESET:
			if (span->ind_count < STABLE_IND_COUNT) break;
			if (span->ci_cmd != IBC_LT_DR) {
				ibc_cmd(span, IBC_LT_DR);
			} else {
				if ((span->ci_ind == IBC_LT_DA) && (span->ibcmode != SHUTDOWN)) {
					ibc_cmd(span, IBC_LT_ARL);
					span->ibcstate = INTERIM_LOOP;
				}
			}
			break;
/* terminal won't wakeup the link unless will see our info, so send them out by switching on the local loop */
		case INTERIM_LOOP:
			if (span->ci_cmd != IBC_LT_ARL) {
				span->ibcstate = CHANGE_STATE;
				break;
			}
			if (span->ind_count < STABLE_IND_COUNT) break;
			if (span->ci_ind != IBC_LT_AIU) break;
			switch (span->ibcmode) {
				case RUN:
					ibc_cmd(span, IBC_LT_ARN);
					span->ibcstate = ACTIVE;
					break;
				case LOCAL_LOOP:
					span->ibcstate = ACTIVE;
					break;
				case REMOTE_LOOP:
					ibc_cmd(span, IBC_LT_AR2);
					span->ibcstate = ACTIVE;
					break;
				case SHUTDOWN:
				default:
		...
		case ACTIVE:
			if (span->ind_count < STABLE_IND_COUNT) break;
			alrm = DAHDI_ALARM_NONE;
			if (span->ci_ind != IBC_LT_AIU) alrm = DAHDI_ALARM_RED;
			if ((span->ibcmode == LOCAL_LOOP) || (span->ibcmode == REMOTE_LOOP)) alrm |= DAHDI_ALARM_LOOPBACK;
			span->newalarm = alrm;
			break;
		case CHANGE_STATE:
		default:
			ibc_cmd(span, IBC_LT_RES);
			span->ibcstate = RESET;
			break;
	}
	spin_unlock_irqrestore(&b4->fifolock, irq_flags);
...
static int hfc_process_ci(struct b4xxp_span *span, unsigned char ind)
{
	span->ci_prev_ind = span->ci_curr_ind;
	span->ci_curr_ind = ind;
	if ((span->ci_prev_ind == ind) && (span->ci_ind != ind)) {
		span->ci_ind = ind;
		span->ind_count = 1;
#if 0
		if (DBG) {
		...
		tmp = span->ind_count + 1;
		if (tmp < MAX_IND_COUNTER) span->ind_count = tmp;
		return -1;
	}

}

static void hfc_process_4ci(struct b4xxp_span *span, unsigned int data)
{
	int	i;
	unsigned char ind;
	...
	for (i = 0; i < 4; i++) {
		ind = (data >> (i * 8)) & 0xff;
		ind = ( ind & IBC_CI_MASK) >> IBC_CI_SHIFT;
		(void)hfc_process_ci(span, ind);
	}
}
...
/* TODO: error checking, full FIFO mostly */
	...
	if (zlen >= DAHDI_CHUNKSIZE) {
		unsigned volatile int tmp;
		
		tmp = b4xxp_getreg32(b4, A_FIFO_DATA2);
		if (muted) tmp = 0xffffffff;
		*(unsigned int *)&chan->readchunk[0] = tmp;
#ifdef DEBUG_SNOOP_CICHAN
		*(unsigned int *)&chan->readchunk[0] = span->ci_ind;
#endif
		tmp = b4xxp_getreg32(b4, A_FIFO_DATA2);
		if (muted) tmp = 0xffffffff;
		*(unsigned int *)&chan->readchunk[4] = tmp;
/*
 * now TX FIFO
 *
 ...
 */
		...
		if (milliwatt) {
			unsigned int const *mwt;
			
			mwt =  ((span->span.deflaw == DAHDI_LAW_MULAW) ? mwt_ulaw : mwt_alaw);
			
			b4xxp_setreg32(b4, A_FIFO_DATA2, mwt[0]);
			b4xxp_setreg32(b4, A_FIFO_DATA2, mwt[1]);
		} else {
...
static int hfc_poll_cichan_fifo(struct b4xxp_span *span)
{
	...
	ret = 0;
	b4 = span->parent;
	fifo = span->fifos[WCB4XXP_CICHAN_FIFO_IDX];
 ...
 */
		...
		b4xxp_setreg32(b4, A_FIFO_DATA2, tmp);
		b4xxp_setreg32(b4, A_FIFO_DATA2, tmp);
		span->cmd_sent = true;
		ret = 1;
	}
...
/* TODO: Make sure S/T port is in active state */
		...
		if (b4->spans[span].flags & SPAN_FLAG_CI) {
			(void)hfc_poll_cichan_fifo(&b4->spans[span]);
		}
 ...
 */
	for (span=0; span < b4->numspans; span++) {
		synthetic = (pcmbus_devs > 0) && (span >= (b4->numspans - pcmbus_devs));
		if (IS_B430P(b4)) {
			...
			if ((vpmsupport) && (CARD_HAS_EC(b4))) {
				...
				if (synthetic) {
					hfc_assign_bchan_fifo_pcmbus(b4, span, 0);
					hfc_assign_bchan_fifo_pcmbus(b4, span, 1);
				} else {
			...
			if (synthetic) {
				hfc_assign_dchan_fifo_pcmbus(b4, span);
#warning Add checks that C/I chan is actually required for this span - via flag from config or whatever
				hfc_assign_cichan_fifo_pcmbus(b4, span);
			} else {
				hfc_assign_dchan_fifo(b4, span);
			}
		}
...
/* init DTMF controller */
static void hfc_init_dtmf(struct b4xxp *b4)
{
	if (!hw_dtmf) return;
	b4->dtmf.base = 0x1000;
	b4->dtmf.cfg = (V_DTMF_EN | V_DTMF_RX_CH | V_DTMF_STOP);
	if (!strcasecmp(companding, "ulaw")) {
		b4->dtmf.cfg |= V_ULAW_SEL;
	}
	b4xxp_setreg8(b4, R_DTMF_N, DSP_DTMF_NPOINTS - 1);
	b4xxp_setreg8(b4, R_DTMF, b4->dtmf.cfg | V_RST_DTMF);
	if (unlikely(DBG_DTMF)) {
		printk("HW DTMF initialized: 0x%0x\n", b4->dtmf.cfg);
	}
}

static void print_dtmf_intermediate(int32_t *result, int thresh)
{
	int32_t thresh_100;
	
	thresh_100 = thresh/100;
	if (thresh_100 == 0) {
		thresh_100 = 1;
		printk("threshold %d too small - thresh/100 is set to 1\n", thresh);
	}
	printk("a %3d %3d %3d %3d %3d %3d %3d %3d"
	       " tr:%3d r %3d %3d %3d %3d %3d %3d %3d %3d\n",
	       result[0] / 10000, result[1] / 10000, result[2] / 10000,
	       result[3] / 10000, result[4] / 10000, result[5] / 10000,
	       result[6] / 10000, result[7] / 10000, thresh / 10000,
	       result[0] / (thresh_100), result[1] / (thresh_100),
	       result[2] / (thresh_100), result[3] / (thresh_100),
	       result[4] / (thresh_100), result[5] / (thresh_100),
	       result[6] / (thresh_100), result[7] / (thresh_100));
}

static inline void compute_cos2pik(int32_t *hfccoeff, int32_t *result)
{
	int	k;
	int32_t	sk, sk2;
	...
	for (k = 0; k < NCOEFF; k++) {
		...
		if (sk > 32767 || sk < -32767 || sk2 > 32767 || sk2 < -32767) 
			printk("DTMF-Detection overflow\n");
		/* compute |X(k)|**2 */
		result[k] = (sk * sk) - 
			(((cos2pik[k] * sk) >> 15) * sk2) +
			(sk2 * sk2);
...
static inline int locate_digit(int32_t *result, int32_t thresh)
{
	int	digit = 0;
	int32_t	threshl;
	...
	lowgroup = -1;
	highgroup = -1;
	threshl = thresh >> 3;  /* tones which are not on, must be below 9 dB */
	thresh = thresh >> 2;  /* touchtones must match within 6 dB */
	for (i = 0; i < NCOEFF; i++) {
		if (result[i] < threshl) continue;  /* ignore */
		if (result[i] < thresh) {
			lowgroup = -1;
			highgroup = -1;
	...
	/* good level found. This is allowed only one time per group */
		...
		/* higroup */
			...
			/* Bad. Another tone found. */
				highgroup = -1;
				break;
			} else highgroup = i - (NCOEFF / 2);
		}
	}

		/* get digit or null */
	if (lowgroup >= 0 && highgroup >= 0)
		digit = dtmf_matrix[lowgroup][highgroup];
	return	digit;
}

static inline char tone2ascii(int tone)
{
	tone -= DAHDI_TONE_DTMF_BASE;
	if (tone < 0) tone = 0;
	if (tone > 15) tone = 15;
	return tone_ascii_tab[tone];
}


static void extract_dtmf_digits(struct dahdi_chan *zapchan, struct chan_dtmf *cd)
{
	int rawdigit;
	int32_t	result[NCOEFF];
	int32_t	thresh;
	int32_t	*hfccoeff;
	int	i, len;
	...
	len = cd->coeff_count;
	while (len >= NCOEFF * 2) {
		compute_cos2pik(hfccoeff, result);
		hfccoeff += (NCOEFF * 2);
		len -= (NCOEFF * 2);
		thresh = 0;
		for (i = 0; i < NCOEFF; i++) {
			if (result[i] < 0) result[i] = 0;
			if (result[i] > cd->threshold) {
				if (result[i] > thresh)	thresh = result[i];
			}
		}
		if (thresh == 0) {
			rawdigit = 0;
		} else {
			if (unlikely((DBG_DTMF) && (DEBUG_ME_HARDER))) print_dtmf_intermediate(result, thresh);
			rawdigit = locate_digit(result, thresh);
		}

//		if (unlikely(rawdigit && (DBG_DTMF))) printk("DTMF rawdigit: %0x\n", rawdigit);

		if (cd->lastraw != rawdigit) cd->raw_count = 0;

	/* the tone (or no tone) must remain 3 times without change */
		if (cd->raw_count == 2) {
			if (cd->lastdigit != rawdigit) {
				if (rawdigit) {
					if (unlikely(DBG_DTMF)) printk("DTMF digit: %c\n", tone2ascii(rawdigit));
					dahdi_qevent_lock(zapchan, DAHDI_EVENT_DTMFUP | rawdigit);
				} else {
					dahdi_qevent_lock(zapchan, DAHDI_EVENT_DTMFDOWN | cd->lastdigit);
				}
				cd->lastdigit = rawdigit;
			}
		} else cd->raw_count++;
		cd->lastraw = rawdigit;
	...
	if (len > 0) {
		printk("%s: coefficients have invalid "
			"size. (is=%d < must=%d)\n",
			__func__, len, 64);
	}
}

static void hfc_read_dtmf(struct b4xxp *b4)
{
	int	span, co, ch, bchan, enabled;
	int32_t	*cfptr;
	unsigned long flags;
	struct chan_dtmf *cd;
	struct dahdi_chan *zapchan;

	for (span=0; span < b4->numspans; span++) {
		if (!(b4->spans[span].span.flags & DAHDI_FLAG_RUNNING)) continue;
		for (bchan = 0; bchan < WCB4XXP_BCHANS_PER_SPAN; bchan++) {
			cd = &(b4->spans[span].tonecfg[bchan]);
			if (!dchan_dtmfevt) {
				zapchan = b4->spans[span].chans[bchan];
			} else {
				zapchan = b4->spans[span].sigchan;
			}
			ch = cd->chan;
			enabled = cd->dtmfenable;
			if ((!enabled) || (ch < 0)) continue;
			cfptr = cd->coeff + cd->coeff_count;
			for (co = 0; co < NCOEFF; co++) {
				int	w, w1;
				
				spin_lock_irqsave(&b4->seqlock, flags);
				set_dtmf_addr(b4, co, ch);
				w = unpack_dtmf_float(read_dtmf_float(b4));
				cfptr[(co << 1)] = w;
				w1 = unpack_dtmf_float(read_dtmf_float(b4));
				cfptr[(co << 1) | 1] = w1;
				spin_unlock_irqrestore(&b4->seqlock, flags);
				if (unlikely(((DBG_DTMF) && (DEBUG_ME_HARDER)))) {
					printk("DTMF coeffs for chan %d, no%d = %0d, %0d\n", ch, co, w, w1);
				}
			}
			cd->coeff_count += NCOEFF * 2;
			if (cd->coeff_count >= DTMF_FRAME_COEFFS) {
			
				extract_dtmf_digits(zapchan, cd);
				cd->coeff_count = 0;
			}
		}
	}
...
//	struct b4xxp *b4 = bspan->parent;
	...
	if (!is_synthetic(bspan)) return -ENOSYS;

	switch(cmd) {
		case DAHDI_MAINT_NONE:
		case DAHDI_MAINT_LOOPDOWN:
			hfc_new_ibc_mode(bspan, RUN);
			break;
		case DAHDI_MAINT_LOCALLOOP:
			hfc_new_ibc_mode(bspan, LOCAL_LOOP);
			break;
		case DAHDI_MAINT_LOOPUP:
			hfc_new_ibc_mode(bspan, REMOTE_LOOP);
			break;
		default:
			return -ENOSYS;
	}
...
static int b4xxp_ioctl(struct dahdi_chan *chan, unsigned int cmd, unsigned long data)
{
	...
	case DAHDI_TONEDETECT:
		if (get_user(val, (__user int *) data)) return -EFAULT;
		if (!hw_dtmf) return -ENOSYS;
		if (chan->chanpos > WCB4XXP_BCHANS_PER_SPAN) {
			printk(KERN_WARNING "Cannot control toneconfig on non-Bchannel %0d of span %d; failing request\n", chan->chanpos, chan->span->offset);
			return -EINVAL;
		}
		if (!hw_rxmute) val &= ~(DAHDI_TONEDETECT_MUTE);
		if (val & DAHDI_TONEDETECT_ON) {
			if (unlikely(DBG_DTMF)) printk("HW DTMF detect enabled on span %0d, chan %0d\n", 
				chan->span->offset, chan->chanpos);
			if (dtmfthreshold < 20 || dtmfthreshold > 500) dtmfthreshold = 200;
			bspan->tonecfg[chan->chanpos - 1].threshold = dtmfthreshold * 10000;
			bspan->tonecfg[chan->chanpos - 1].dtmfenable = true;
		} else {
			if (unlikely(DBG_DTMF)) printk("HW DTMF detect disabled on span %0d, chan %0d\n", 
				chan->span->offset, chan->chanpos);
			bspan->tonecfg[chan->chanpos - 1].dtmfenable = false;
		}
		if (val & DAHDI_TONEDETECT_MUTE) {
			if (unlikely(DBG_DTMF)) printk("RX muted on span %0d, chan %0d\n", 
				chan->span->offset, chan->chanpos);
			bspan->tonecfg[chan->chanpos - 1].rxmuted = true;
		} else {
			if (unlikely(DBG_DTMF)) printk("RX unmuted on span %0d, chan %0d\n", 
				chan->span->offset, chan->chanpos);
			bspan->tonecfg[chan->chanpos - 1].rxmuted = false;
		}
		return 0;
...
static void b4xxp_reset_span(struct b4xxp_span *bspan)
{
	...
	if (bspan->flags & SPAN_FLAG_CI) {
		hfc_reset_fifo_pair(b4, bspan->fifos[WCB4XXP_CICHAN_FIFO_IDX], 0, 1);
	}
	for (i=0; i < 3; i++) {
			    ...
			    struct dahdi_lineconfig *lc)
{
	...
	struct b4xxp_span *bspan = container_of(span, struct b4xxp_span, span);
	struct b4xxp *b4 = bspan->parent;
	bool synthetic = is_synthetic(bspan);
	...
	if (synthetic) lc->sync = 0;	/* PCMBUS spans cannot be timing sources */
...
static void b4xxp_bottom_half(unsigned long data)
{
	...
	if (IS_GEN2(b4)) {
		...
		for (j = 0; j < b4->numspans; j++) {
			...
			if ( b & (V_IRQ_FIFOx_TX << roll)) {
				b2 &= ~(V_IRQ_FIFOx_TX << roll);
	/*		
 ...
 */
		...
		if (!(b4->ticks % 500)) b4xxp_update_ibc(b4);
...
/* clear the timer interrupt flag. */
	...
	if (b4->misc_irqstatus & V_DTMF_IRQ) {
		if (hw_dtmf) hfc_read_dtmf(b4);
		b4->misc_irqstatus &= ~V_DTMF_IRQ;
	}
/*
...
*/
	...
	if (pcmbus_devs > MAX_PCMBUS_DEVS) {
		pcmbus_devs = MAX_PCMBUS_DEVS;
		if (DBG) dev_info(&pdev->dev, "pcmbus_devs cannot be > %d, set to %d\n", 
					MAX_PCMBUS_DEVS, MAX_PCMBUS_DEVS);
	}
	if ((dt->flags & FLAG_CANPCMBUS) && (pcmbus_devs > 0)) {
		dev_info(&pdev->dev, "PCMBUS devices connected as %d additional span(s), hw ec disabled\n", pcmbus_devs);
		b4->numspans = dt->ports + pcmbus_devs;
		vpmsupport = 0;
	} else {
		b4->numspans = dt->ports;
	}

	if (dt->flags & FLAG_CANHWDTMF) {
		dev_info(&pdev->dev, "Hardware DTMF detector supported %s.\n", 
			hw_dtmf ? "and enabled" : "but disabled" );
	} else {
		hw_dtmf = 0;
	}
...
static void b4xxp_proc_show_one(struct seq_file *m, struct b4xxp *b4)
{
	struct dahdi_chan *chan;
	int i, j;
	
	seq_printf(m, "Card %d, IRQ %d\n", b4->cardno + 1, b4->irq);

	seq_printf(m,"\nTx:\n");
	for (j=0; j<(b4->numspans * 2) ; j++) {			/* B Channels */
		for (i=0; i<(b4->numspans * 3) ; i++) {		/* All Channels */
			chan = b4->spans[i/3].chans[i%3];
			seq_printf(m, "%02x ", chan->writechunk[j]);
		}
		seq_printf(m, "\n");
	}
	...
	for (j=0; j < (b4->numspans * 2); j++) {		/* B Channels */
		for (i=0; i < (b4->numspans * 3); i++) { 	/* All Channels */
			chan = b4->spans[i / 3].chans[i % 3];
			seq_printf(m, "%02x%c", chan->readchunk[j], (i == 11) ? '\n' : ' ');
		}
	}
	...
	for (i=0; i < b4->numspans; i++) {
		...
		if (is_synthetic(s)) {
			nt = !s->te_mode;
			seq_printf(m, "P%d: %s on PCMBUS (synthetic)", s->port + 1, (nt ? "NT" : "TE"));
			seq_printf(m, "   IBC mode: %s\n    IBC state: %s\n    CMD: 0x%02x, IND: 0x%02x, IND count: %d\n", 
				ibc_modename(s->ibcmode), ibc_statename(s->ibcstate),
				s->ci_cmd, s->ci_ind, s->ind_count);
		} else {
		...
		seq_printf(m, "    Tone config: B1 - RX %s/DTMF %s, B2 - RX %s/DTMF %s\n",
				s->tonecfg[0].rxmuted ? "MUTED" : "UNMUTED",
				s->tonecfg[0].dtmfenable ? "ON" : "OFF",
				s->tonecfg[1].rxmuted ? "MUTED" : "UNMUTED",
				s->tonecfg[1].dtmfenable ? "ON" : "OFF" );
	}
	seq_printf(m, "\nTicks: %ld\n", b4->ticks);
}

static int b4xxp_proc_show(struct seq_file *m, void *v)
{
	struct b4xxp **b4_cards = cards;
	int	i;

	seq_printf(m, "WCB4XXP card information\n");
	for (i=0; b4_cards[i] != NULL; i++) {
		if (i) seq_printf(m, "\n-----\n");
		b4xxp_proc_show_one(m, b4_cards[i]);
	}
	seq_printf(m, "\n-----\n\nAudio: ");
#ifdef LOOPBACK_SUPPORTED
	if (loopback >= 3)
		seq_printf(m, "DAHDI and S/T\n");
	else if (loopback == 2)
		seq_printf(m, "DAHDI\n");
	else if (loopback == 1)
		seq_printf(m, "S/T\n");
		...
		seq_printf(m, "not");
	seq_printf(m, " looped back\n");
#else
	seq_printf(m, "not looped back\n");
#endif
	seq_printf(m, "dbgval1=0x%0x, dbgval2=0x%0x\n", dbgval1, dbgval2);
	if (milliwatt)
		seq_printf(m, "Outgoing S/T replaced with milliwatt tone\n");
	if (alarmdebounce)
		seq_printf(m, "Alarms: debounced (%dms)\n", alarmdebounce);
	else
		seq_printf(m, "Alarms: not debounced\n");
	seq_printf(m, "T1 timer period %dms\nT3 timer period %dms\n", timer_1_ms, timer_3_ms);

	return 0;
}

static int b4xxp_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, b4xxp_proc_show, NULL);
}

static const struct file_operations b4xxp_proc_fops = {
	.owner = THIS_MODULE,
	.open = b4xxp_proc_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = seq_release,
};
...
#ifdef CREATE_WCB4XXP_PROCFS_ENTRY
	myproc = proc_create(PROCFS_NAME, 0, NULL, &b4xxp_proc_fops);
	if (!myproc) {
...
#endif
module_param(dbgval1, uint, S_IRUGO | S_IWUSR);
module_param(dbgval2, uint, S_IRUGO | S_IWUSR);
module_param(hw_dtmf, uint, S_IRUGO);
module_param(hw_rxmute, uint, S_IRUGO);
module_param(dtmfthreshold, int, S_IRUGO | S_IWUSR);
module_param(dchan_dtmfevt, int, S_IRUGO | S_IWUSR);
...
module_param(pcmbus_devs, int, S_IRUGO);
module_param(pcm_loop, int, S_IRUGO);
module_param(pcm_slot_loop, int, S_IRUGO);
module_param(pcm_synci, int, S_IRUGO);
module_param(pcm_c4_pol, int, S_IRUGO);
module_param(pcm_f0_neg, int, S_IRUGO);
module_param(pcm_f0_len, int, S_IRUGO);
...
#endif
MODULE_PARM_DESC(dbgval1, "debug value 1 for different purposes. For developers only");
MODULE_PARM_DESC(dbgval2, "debug value 2 for different purposes. For developers only");
MODULE_PARM_DESC(milliwatt, "1=replace outgoing S/T data with milliwatt");
...
MODULE_PARM_DESC(companding, "Change the companding to \"alaw\" or \"ulaw\""\
				"(alaw by default)");
MODULE_PARM_DESC(master, "1=the card runs in master mode (default is no)");
MODULE_PARM_DESC(hw_dtmf, "1=enable hardware dtmf detector (default is no)");
MODULE_PARM_DESC(hw_rxmute, "1=enable hardware RX mute (not DTMF mute) (default is no)");
MODULE_PARM_DESC(dtmfthreshold, "dtmf detection threshold. Can be from 20 to 500. default is 100");
MODULE_PARM_DESC(dchan_dtmfevt, "1=report DTMF events on D-channel instead of B-channel (default is no)");
MODULE_PARM_DESC(pcm_loop, "1=all pcmbus data is looped back internally (default is no)");
MODULE_PARM_DESC(pcm_slot_loop, "1=pcmbus data are looped back for all B-channels (default is no)");
MODULE_PARM_DESC(pcm_synci, "1=SYNC_I is used as PCM bus synch signal (default is no)");
MODULE_PARM_DESC(pcm_c4_pol, "1=F0IO is sampled on positive C4IO transition, 0=(the default) is on negative C4IO transition");
MODULE_PARM_DESC(pcm_f0_neg, "1=F0IO is negative pulse, 0 (the default) is positive");
MODULE_PARM_DESC(pcm_f0_len, "1=F0IO is active for two C4IO clocks, 0 - (the default) for one C4IO clock");
MODULE_PARM_DESC(pcmbus_devs, "number of pcmbus devices to be seen as additional spans"\
				" (up to 5 max, default is 0");

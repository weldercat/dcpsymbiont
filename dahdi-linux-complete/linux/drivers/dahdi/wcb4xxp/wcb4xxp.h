...
/* R_RAM_ADDR2 bits */
#define V_ADDR_RES		(1 << 6)	/* address reset */
#define V_ADDR_INC		(1 << 7)	/* address increment */
...
#define V_SYNC_PLL		(1 << 1)
#define V_SYNC_SRC		(1 << 2)
#define V_SYNC_OUT		(1 << 3)
#define V_PLL_ICR		(1 << 6)
#define V_PLL_MAN		(1 << 7)
...
/* R_DTMF bits */
#define V_DTMF_EN		(1 << 0)	/* 1=enable DTMF unit */
#define V_HARM_SEL		(1 << 1)	/* 1=16 frequencies in 16 channels, 0=8 frequencies in 32 channels */
#define V_DTMF_RX_CH		(1 << 2)	/* 1=detect DTMF in rx channels */
#define V_DTMF_STOP		(1 << 3)	/* 1=DTMF processing stops after n samples */
#define V_CHBL_SEL		(1 << 4)	/* 1=upper 16 channels */
#define V_RST_DTMF		(1 << 6)	/* 1=restart DTMF processing */
#define V_ULAW_SEL		(1 << 7)	/* 1=use ulaw format */
...
/* PEB2095 IBC (ISDN burst controller) C/I channel defines */

#define IBC_CI_SHIFT	2	/* C/I channel bits position in B* cahnnel byte */
#define IBC_CI_MASK	(0xf << 2)

/* LT mode commands */
#define	IBC_LT_DR	0x00	/* Deactivate Request */
#define IBC_LT_ARN	0x08	/* Activate request No loop */
#define	IBC_LT_ARL	0x09	/* Activate request Local loop */
#define	IBC_LT_AR2	0x0a	/* Activate Request loop 2 */
#define IBC_LT_DID	0x0f	/* Deactivate indication */
#define IBC_LT_RES	0x0d	/* software Reset */
#define IBC_LT_DIS	0x03	/* Disable Supply */
#define IBC_LT_SSP	0x05	/* Send Single Pulses - 4kHz AMI ones transmitted */
#define IBC_LT_SCP	0x07	/* Send Continious Pulses - AMI ones transmitted continiously */

/* LT mode indications */
#define IBC_LT_DIU	0x0f	/* Deactivate Indication */
#define IBC_LT_DA	0x01	/* Deactivate Acknowledge */
#define IBC_LT_RDS	0x07	/* Running Digital Sum code viol. register enabled. Receiver synchronized */
#define IBC_LT_ARU	0x08	/* Activate Request */
#define IBC_LT_AIU	0x0c	/* Activate Indication */
#define IBC_LT_RSYU	0x04	/* Resynchronization - Lost framing: receiver not synced to input signal */
#define IBC_LT_HI	0x03	/* PWR supply becomes high impedance after PFOFF */

/* PT/TE mode commands */
#define	IBC_PT_TIM	0x00	/* Layer 2 requests clock to be activated */
#define IBC_PT_ARN	0x08	/* Activate Request no Loop */
#define IBC_PT_ARL	0x09	/* Activate request local loop */
#define IBC_PT_AIN	0x0c	/* Activate Indication */
#define IBC_PT_DIU	0x0f	/* Deactivate indication */
#define IBC_PT_RES	0x0d	/* Software Reset */
#define IBC_PT_SSP	0x05	/* Send single pulses */
#define IBC_PT_SCP	0x07	/* Send Continious Pulses */

/* PT/TE mode indications */
#define IBC_PT_DR	0x00	/* Disconnect Request */
#define IBC_PT_PU	0x07	/* Power Up */
#define IBC_PT_RSYU	0x04	/* Receiver not synchornized with input */
#define IBC_PT_ARN	0x08	/* Activate request no loop */
#define IBC_PT_ARL	0x09	/* Activate request local loop */
#define IBC_PT_AR2	0x0a	/* Activate request loop 2 */
#define IBC_PT_AIN	0x0c	/* Activate indication no loop */
#define IBC_PT_AIL	0x0d	/* Activate indication local loop */
#define IBC_PT_AI2	0x0e	/* Activate indication loop 2 */
#define IBC_PT_TMA	0x05	/* Test mode acknowledge */
#define IBC_PT_DID	0x0f	/* Deactivate indication */
...
#define MAX_SPANS_PER_CARD			10
#define MAX_PCMBUS_DEVS				5	/* maximum PCMBUS devices (IBCs) supported */

#define WCB4XXP_BCHANS_PER_SPAN			2
#define WCB4XXP_CHANNELS_PER_SPAN		3	/* 2 B-channels and 1 D-Channel for each BRI span */
#define WCB4XXP_FIFOS_PER_SPAN			4	/* 2 B-channels, 1 D-channel and possibly C/I chan */
#define WCB4XXP_HDLC_BUF_LEN			32	/* arbitrary, just the max # of byts we will send to DAHDI per call */

#define	SPAN_FLAG_PCMBUS	(1 << 0)	/* this span is interfaced to PCM bus only. No physical port */
#define SPAN_FLAG_CI		(1 << 1)	/* this span has C/I channel to control layer 1 phy */

#define WCB4XXP_BCHAN0_FIFO_IDX	0
#define WCB4XXP_BCHAN1_FIFO_IDX	1
#define WCB4XXP_DCHAN_FIFO_IDX	2		/* d-channel fifo IDX for each span */
#define WCB4XXP_CICHAN_FIFO_IDX	3		/* C/I channel fifo IDX for each span */		

#define MAX_IND_COUNTER		1000000
#define STABLE_IND_COUNT	100
...
#define DSP_DTMF_ITERATIONS	8
#define DTMF_FRAME_COEFFS	128
#define DTMF_ENQUED_EVENTS	16
...
struct chan_dtmf {
	...
	int	raw_count;
	int	coeff_count;
	int32_t	coeff[DTMF_FRAME_COEFFS];
	int	lastdigit;
	int	lastraw;
};
...
struct b4xxp_span {
	...
	unsigned int flags;			/* internal span flags */
	...
	unsigned char ci_cmd;			/* command to be sent to the C/I channel */
	unsigned char ci_prev_ind;		/* previous command seen from C/I channel */
	unsigned char ci_curr_ind;		/* current command seen from C/I channel */
	unsigned char ci_ind;			/* decoded C/I command (two last repetitions) */
	int cmd_sent;				/* a new ci cmd was sent at least two times */
	int ind_count;				/* indication repetition counter */
	enum ibc_mode ibcmode;			/* requested ibc mode (for pcmbus only */
	enum ibc_state	ibcstate;			/* current ibc state (fro pcmbus only) */
	...
	int fifos[WCB4XXP_FIFOS_PER_SPAN];				/* B1, B2, D <--> host fifo numbers */
	struct chan_dtmf tonecfg[WCB4XXP_BCHANS_PER_SPAN];
...
struct b4xxp {
	...
	struct dtmf_config dtmf;			/* hw dtmf detector related stuff */

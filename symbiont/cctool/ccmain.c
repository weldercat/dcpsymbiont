...
static void dcp_event_enq(struct mmi_event *evt);
...
int ccinit(const char *cfgfile)
{
	...
	assert(cfgfile);
	res= cfload(confdb, &gparm, (char *)cfgfile);
	if (res != SYM_OK) {
 ...
 */
	...
	if (st_evt_filter_pass(st, evt)) dcp_event_enq(evt);
...
int dcp_command_hndlr(struct conn_ctx_s *ctx, yatemsg *msg, void *arg, struct dwhook *dwh)
{
	...
	if (ctlname) cmd.ctlname = bfromcstr(ctlname);
	switch (cmd.type) {
		case MMI_CMD_LED:
			if (!ctlname) {
...
#warning real preselector must acquire/release line mutex

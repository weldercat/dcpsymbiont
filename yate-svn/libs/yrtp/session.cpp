...
//    if (typ == eventPayload()) {
//	DDebug(DebugInfo,"RTP preliminary trace mrk=%d, type=%d, tstamp=%d, len=%d",
//	    marker, typ, m_tsLast, len);
//    }
    if (m_dejitter) {
    ...
    if (ds >= 1) {
//	if (typ == eventPayload()) {
//	    DDebug(DebugInfo,"RTP is about to process pkt mrk=%d, type=%d, tstamp=%d, len=%d",
//	    marker, typ, m_tsLast, len);
//    	}
...
bool RTPReceiver::decodeEvent(bool marker, unsigned int timestamp, const void* data, int len)
{
	...
	m_evVol = vol;
//	DDebug(DebugInfo,"RTPReceiver::decodeEvent() raw=0x%0x,0x%0x,0x%0x,0x%0x ev=%d, vol=%d, end=%d, drtn=%d, m_evTs=%d, m_evNum=%d, tstamp=%d ",
//		pc[0],pc[1],pc[2],pc[3], event, vol, end, duration, m_evTs, m_evNum, timestamp);
	if (!end) {
	    m_evTs = timestamp;
	    m_evNum = event;
	    continue;
...
//	DDebug(DebugInfo, "RTPReceiver::decodeEvent() pushEvent called - ev=%d, drtn=%d, vol=%d, tstmp=%d",
//		event, duration, vol, timestamp);
    }
    return true;
}
...
{
//    DDebug(DebugInfo, "RTPReceiver::finishEvent() called with tstamp=%d, evNum=%d, evTs=%d, drtn=%d",
//	    timestamp, m_evNum, m_evTs, (timestamp - m_evTs));
    if ((m_evNum < 0) || !m_evTs)
	return;
...
{
    DDebug(DebugInfo,"RTPSession::rtpRecvEvent(%d,%02x,%d,%d,%u) [%p]",

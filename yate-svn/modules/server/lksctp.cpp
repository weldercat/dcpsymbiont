...
int LKSocket::recvMsg(void* buf, int length, SocketAddr& addr, int& stream, int& flags)
{
	...
	m_error = errno;
	if (!canRetry()) {
	    DDebug(&plugin,DebugAll,"sctp_recvmsg() error: %s, Socket::valid() = %d, alive()=%d, m_error = %d", strerror(errno), Socket::valid(), alive(), m_error);
	}
    }
...
//    DDebug(&plugin, DebugNote, "localUp=%d, status.sstat_state=%d, status.sstat_primary.spinfo_state=%d", 
//		localUp, status.sstat_state, status.sstat_primary.spinfo_state);
    if (localUp)
	return true;

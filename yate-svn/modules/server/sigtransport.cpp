...
bool StreamReader::readData()
{
	...
	return false;
    if (m_reconnect) {
    	DDebug(m_transport,DebugWarn,"downing the conn due to m_reconnect");
	...
	if (m_transport->transType() == Transport::Sctp) {
		...
		DDebug(m_transport,DebugWarn,"downing the conn due to flags != 2, flags=%d", flags);
		connectionDown();
		return false;
	    }
	} else {
	...
	if (len == 0) {
	    DDebug(m_transport,DebugWarn,"downing the conn due to zero-length sctp msg");
	    connectionDown();
	    return false;
	}
	if (len < 0) {
	    if (!m_socket->canRetry()) { 
	    	DDebug(m_transport,DebugWarn,"downing the conn due to negative len, and socket being not retryable, len=%d", len);
		connectionDown();
	    }
	    return false;
	}
	...
	if (m_transport->transType() == Transport::Sctp) {
	    ...
	    if (flags && flags != 2) {
		Debug(m_transport,DebugWarn,"downing the conn due to unexpected flags value=%d", flags);
		connectionDown();
		return false;
	    }
	}
	...
	if (len == 0) {
	    if (!m_transport->supportEvents()) {
	    	DDebug(m_transport,DebugWarn,"downing the conn due to zero-len packet and transport not supporting events");
		connectionDown();
	    }
	    return false;
	}
	if (len < 0) {
	    if (!m_socket->canRetry()) {
	    	DDebug(m_transport,DebugWarn,"downing the conn due negative len 2, and socket being not retryable, len=%d", len);
		connectionDown();
	    }
	    return false;
	}

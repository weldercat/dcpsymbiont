...
bool ZapDevice::setEchoCancel(bool enable, unsigned int taps)
{
    enable = enable && taps;
    int tmp = 1;
    if (enable && (type() == E1 || type() == T1 || type() == BRI) &&
...
// Check if received data. Wait usec microseconds before returning
bool ZapDevice::select(int usec)
{
    FD_ZERO(&m_rdfds);
    ...
    m_tv.tv_sec = 0;
    m_tv.tv_usec = (usec >= 0) ? usec : 0;
    int sel = ::select(m_handle+1,&m_rdfds,NULL,&m_errfds,(usec >= 0) ? &m_tv : NULL);
    if (sel >= 0) {
    ...
    m_errno(0)
{
	...
	case ZapDevice::BRI:
		...
		m_format = (ZapDevice::Format)lookup(f,s_formats,ZapDevice::Alaw);
